...
export function ActivityTimeline() {
      ...
      <div className="space-y-6">
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} className="flex gap-4 animate-pulse">
            <div className="w-2 bg-surface rounded-full h-12" />
  ...
  if (!activities || activities.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center opacity-40">
        <Clock className="w-12 h-12 mb-4 text-forest" />
        <p className="font-display font-bold text-lg text-forest">No activity yet</p>
        <p className="text-sm text-charcoal-light">New bookings and expenses will appear here.</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
          ...
          return (
            ...
            >
              ...
              <div className="relative mt-1.5">
                <div className={`w-6 h-6 rounded-full border-4 border-white shadow-sm ${dotColor} z-10 relative`} />
              </div>

              {/* Content */}
              <div className="flex-1 flex justify-between items-center bg-surface/30 p-4 rounded-2xl border border-border-color/50 hover:bg-surface/50 transition-colors group">
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-charcoal line-clamp-1">{item.title}</h4>
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted mt-0.5">{item.subtitle}</p>
                  </div>
                </div>

                <div className="text-right">
                  <p className="text-sm font-display font-bold text-forest italic">
                    {isExpense ? '-' : '+'} KES {item.amount.toLocaleString()}
                  </p>
                  <p className="text-[10px] text-muted font-medium mt-0.5 flex items-center justify-end gap-1">
      ...
      <button className="w-full mt-8 py-3 border border-dashed border-border-color rounded-xl text-xs font-bold text-muted hover:text-gold hover:border-gold transition-all flex items-center justify-center gap-2 group">
        VIEW ALL ACTIVITY
